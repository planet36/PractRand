...
void PractRand::RNGs::Polymorphic::isaac32x256::seed(Uint64 s) {
    implementation.seed(s);
}
void PractRand::RNGs::Polymorphic::isaac32x256::seed(vRNG *seeder_rng) {
    implementation.seed(seeder_rng);
}
void PractRand::RNGs::Polymorphic::isaac32x256::flush_buffers() {
    implementation.flush_buffers();
}
std::string PractRand::RNGs::Polymorphic::isaac32x256::get_name() const {
    return "isaac32x256";
}

PRACTRAND_POLYMORPHIC_RNG_BASICS_C64(isaac64x256)
void PractRand::RNGs::Polymorphic::isaac64x256::seed(Uint64 s) {
    implementation.seed(s);
}
void PractRand::RNGs::Polymorphic::isaac64x256::seed(vRNG *seeder_rng) {
    implementation.seed(seeder_rng);
}
void PractRand::RNGs::Polymorphic::isaac64x256::flush_buffers() {
    implementation.flush_buffers();
}
std::string PractRand::RNGs::Polymorphic::isaac64x256::get_name() const {
    return "isaac64x256";
}

// raw:
PractRand::RNGs::Raw::isaac32x256::~isaac32x256() {
    std::memset(this, 0, sizeof(*this));
}
PractRand::RNGs::Raw::isaac64x256::~isaac64x256() {
    std::memset(this, 0, sizeof(*this));
}

#define ind32(mm, x)                                                           \
    (*reinterpret_cast<Uint32 *>(reinterpret_cast<Uint8 *>(mm) +               \
                                 ((x) & ((SIZE - 1) << 2))))
...
void PractRand::RNGs::Raw::isaac32x256::_seed(
    bool flag) { // LOCKED, do not change
    // the reference seeding algorithm for ISAAC
    // not used directly by PractRand since it isn't a good match for any
    // PractRand interface but the visible seeding methods map to this
    Uint32 tmp[8];
    a = b = c = 0;
    for (auto &i : tmp)
        i = 0x9e3779b9UL; // the golden ratio
...
void PractRand::RNGs::Raw::isaac32x256::seed(
    Uint32 s[256]) { // LOCKED, do not change
    for (int i = 0; i < 256; i++)
        state[i] = s[i];
    _seed(true);
}
void PractRand::RNGs::Raw::isaac32x256::seed(Uint64 s) { // LOCKED, do not
                                                         // change
    // changed in 0.85 to improve seeding quality
    // and to make it more similar to the reference seeding algorithm
    //(which can't be used directly since it takes a non-standard seed
    //structure)
    state[0] = Uint32(s);
    state[1] = Uint32(s >> 32);
...
void PractRand::RNGs::Raw::isaac32x256::seed(
    vRNG *seeder_rng) { // LOCKED, do not change
...
void PractRand::RNGs::Raw::isaac32x256::self_test() {
    ...
    if (checksum != 0x3ff1d629d8878831ull)
        issue_error("isaac32x256::self_test() failed");
}

#define ind64(mm, x)                                                           \
    (*reinterpret_cast<Uint64 *>(reinterpret_cast<Uint8 *>(mm) +               \
                                 ((x) & ((SIZE - 1) << 3))))
...
void PractRand::RNGs::Raw::isaac64x256::_seed(
    bool flag) { // LOCKED, do not change
    ...
    a = b = c = 0;
    for (auto &i : tmp)
        i = 0x9e3779b97f4a7c13uLL; // the golden ratio
...
void PractRand::RNGs::Raw::isaac64x256::seed(
    Uint64 s[256]) { // LOCKED, do not change
    for (int i = 0; i < 256; i++)
        state[i] = s[i];
    _seed(true);
}
void PractRand::RNGs::Raw::isaac64x256::seed(Uint64 s) { // LOCKED, do not
                                                         // change
    // changed in 0.85 to improve seeding quality
    // and to make it more similar to the reference seeding algorithm
    //(which can't be used directly since it takes a non-standard seed
